...
test.describe(`Verify Login Functionality`, () => {
    ...
    test(`Verify the My widget page functionality`, async ({ page }) => {
        ...
        // if the user has never agreed to the widget terms of agreement, agree
        const agreeCheckbox = await page.locator('[type=checkbox]');

        if(agreeCheckbox.isVisible()) {
            agreeCheckbox.click();
            const agreeButton = await page.locator('button:has-text("Agree")')
            agreeButton.click()
        }
